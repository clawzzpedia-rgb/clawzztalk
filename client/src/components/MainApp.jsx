            ...
            {currentServer && !currentDM && (
              <div className="h-12 flex items-center px-4 border-b border-[#1f2023] bg-[#313338] shadow-sm">
                <div className="flex items-center flex-1">
                  <div className="flex items-center gap-1.5 text-[#6d6f78] font-semibold text-sm">
                    <span className="text-[#6d6f78]">#</span>
                    <span className="text-white font-semibold">{currentChannel?.name || 'general'}</span>
                  </div>
                </div>
                {user?.is_admin && (
                  <button onClick={() => setShowAdmin(true)} className="flex items-center gap-1.5 text-xs bg-[#2b2d31] hover:bg-[#35373c] text-[#5865f2] px-2.5 py-1.5 rounded transition font-medium">
          ...
          {showCreate && (
            <div className="absolute inset-0 bg-black/60 flex items-center justify-center z-50" onClick={() => setShowCreate(false)}>
              <div className="bg-[#2b2d31] rounded-lg p-6 w-80" onClick={(e) => e.stopPropagation()}>
                ...
                <input autoFocus value={serverName} onChange={(e) => setServerName(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && createServer()} className="w-full bg-[#1e1f22] text-white px-3 py-2 rounded mb-3 text-sm border border-transparent focus:border-[#5865f2] focus:outline-none" placeholder="Server name" />
          ...
          {showSearch && (
            <div className="absolute inset-0 bg-black/60 flex items-start justify-center pt-20 z-50" onClick={() => setShowSearch(false)}>
              <div className="bg-[#2b2d31] rounded-lg p-4 w-96" onClick={(e) => e.stopPropagation()}>
                ...
                <input autoFocus value={searchQuery} onChange={(e) => searchServers(e.target.value)} className="w-full bg-[#1e1f22] text-white px-3 py-2 rounded mb-2 text-sm border border-transparent focus:border-[#5865f2] focus:outline-none" placeholder="Search servers..." />
                  ...
                  {searchResults.map((s) => (
                    <div key={s.id} className="flex items-center justify-between p-2 hover:bg-[#383a40] rounded cursor-pointer" onClick={() => joinServer(s.id)}>
                      <div className="flex items-center gap-2">
                        <div className="w-8 h-8 rounded-full bg-[#5865f2] flex items-center justify-center text-white text-xs font-bold">{s.name[0].toUpperCase()}</div>
          ...
          {showUserSearch && (
            <div className="absolute inset-0 bg-black/60 flex items-start justify-center pt-20 z-50" onClick={() => setShowUserSearch(false)}>
              <div className="bg-[#2b2d31] rounded-lg p-4 w-96" onClick={(e) => e.stopPropagation()}>
                ...
                <input autoFocus value={userQuery} onChange={(e) => searchUsers(e.target.value)} className="w-full bg-[#1e1f22] text-white px-3 py-2 rounded mb-2 text-sm border border-transparent focus:border-[#5865f2] focus:outline-none" placeholder="Search users..." />
                <div className="max-h-60 overflow-y-auto">
                  {userResults.map((u) => (
